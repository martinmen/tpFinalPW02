...
if(isset($_GET['tipo_vuelo'])){
    ...
    if ($tipo_vuelo == 1){
        ...
        $sql = "SELECT id_estacion, nombre FROM estacion WHERE cod_tipo_vuelo = " . $tipo_vuelo . " UNION SELECT id_estacion, nombre FROM estacion  where id_estacion = 1 UNION SELECT ID_ESTACION, NOMBRE FROM ESTACION WHERE id_estacion = 3 ORDER BY ID_ESTACION ASC; ";
        $resultado = mysqli_query($conexion, $sql);
        while ($fila = mysqli_fetch_assoc($resultado)) {
            echo "<option value='" . $fila["id_estacion"] . "'>" . $fila["nombre"] . "</option>";
        }
